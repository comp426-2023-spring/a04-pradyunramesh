export function rps(shot){
    //Export rps function
    const shot_list = ['rock', 'paper', 'scissors']; //Create shotlist with valid shots
    const random_index = Math.floor(Math.random()*3); // Select random index
    const random_shot = shot_list[random_index]; //Select shot from shot list using index for opponent
    if (shot == undefined){
...
export function rpsls(shot){
    ...
    const shot_list = ['rock', 'paper', 'scissors','lizard','spock']; //Create shotlist with valid shots
    const random_index = Math.floor(Math.random()*3); //Select random index
    const random_shot = shot_list[random_index];//Select shot from shot list using index for opponent
    if (shot == undefined){
        //Undefined shot condition
